...
cvar_t cl_life = {"cl_life", "0"};
cvar_t cl_slowmo = {"cl_slowmo", "5"};
cvar_t cl_activate = {"cl_activate", "0"};
...
void S_DrawCharacter (int cx, int line, int num)
{
	Draw_Character ( cx + ((vid.width - 320)>>1), line, num);
}
void S_Print (int cx, int cy, char *str)
{
	while (*str)
	{
		S_DrawCharacter (cx, cy, (*str)+128);
		str++;
		cx += 8;
	}
}
...
*/
	if (cl.stats[STAT_ACTIVEWEAPON] == IT_SUPER_NAILGUN)
	{
	if (cl.stats[STAT_AMMO] >= 60)
	{

	M_DrawPic ( (626-smgb->width)/2, 36, smgb);
	}

	if (cl.stats[STAT_AMMO] >= 59)
	{

	M_DrawPic ( (632-smgb->width)/2, 36, smgb);
	}

	if (cl.stats[STAT_AMMO] >= 58)
	{

	M_DrawPic ( (638-smgb->width)/2, 36, smgb);
	}

	if (cl.stats[STAT_AMMO] >= 57)
	{

	M_DrawPic ( (644-smgb->width)/2, 36, smgb);
	}

	if (cl.stats[STAT_AMMO] >= 56)
	{

	M_DrawPic ( (650-smgb->width)/2, 36, smgb);
	}

	if (cl.stats[STAT_AMMO] >= 55)
	{

	M_DrawPic ( (656-smgb->width)/2, 36, smgb);
	}

	if (cl.stats[STAT_AMMO] >= 54)
	{

	M_DrawPic ( (662-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 53)
	{

	M_DrawPic ( (668-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 52)
	{

	M_DrawPic ( (674-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 51)
	{

	M_DrawPic ( (680-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 50)
	{

	M_DrawPic ( (686-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 49)
	{

	M_DrawPic ( (692-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 48)
	{

	M_DrawPic ( (698-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 47)
	{

	M_DrawPic ( (704-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 46)
	{

	M_DrawPic ( (710-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 45)
	{

	M_DrawPic ( (716-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 44)
	{

	M_DrawPic ( (722-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 43)
	{

	M_DrawPic ( (728-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 42)
	{

	M_DrawPic ( (734-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 41)
	{

	M_DrawPic ( (740-smgb->width)/2, 36, smgb);
	}
	if (cl.stats[STAT_AMMO] >= 40)
	{

	M_DrawPic ( (626-smgb->width)/2, 43, smgb);
	}

		if (cl.stats[STAT_AMMO] >= 39)
		{

		M_DrawPic ( (632-smgb->width)/2, 43, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 38)
		{

		M_DrawPic ( (638-smgb->width)/2, 43, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 37)
		{

		M_DrawPic ( (644-smgb->width)/2, 43, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 36)
		{

		M_DrawPic ( (650-smgb->width)/2, 43, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 35)
		{

		M_DrawPic ( (656-smgb->width)/2, 43, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 34)
		{

		M_DrawPic ( (662-smgb->width)/2, 43, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 33)
		{

		M_DrawPic ( (668-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 32)
		{

		M_DrawPic ( (674-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 31)
		{

		M_DrawPic ( (680-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 30)
		{

		M_DrawPic ( (686-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 29)
		{

		M_DrawPic ( (692-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 28)
		{

		M_DrawPic ( (698-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 27)
		{

		M_DrawPic ( (704-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 26)
		{

		M_DrawPic ( (710-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 25)
		{

		M_DrawPic ( (716-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 24)
		{

		M_DrawPic ( (722-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 23)
		{

		M_DrawPic ( (728-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 22)
		{

		M_DrawPic ( (734-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 21)
		{

		M_DrawPic ( (740-smgb->width)/2, 43, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 20)
		{

		M_DrawPic ( (626-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 19)
		{

		M_DrawPic ( (632-smgb->width)/2, 50, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 18)
		{

		M_DrawPic ( (638-smgb->width)/2, 50, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 17)
		{

		M_DrawPic ( (644-smgb->width)/2, 50, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 16)
		{

		M_DrawPic ( (650-smgb->width)/2, 50, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 15)
		{

		M_DrawPic ( (656-smgb->width)/2, 50, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 14)
		{

		M_DrawPic ( (662-smgb->width)/2, 50, smgb);
		}

		if (cl.stats[STAT_AMMO] >= 13)
		{

		M_DrawPic ( (668-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 12)
		{

		M_DrawPic ( (674-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 11)
		{

		M_DrawPic ( (680-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 10)
		{

		M_DrawPic ( (686-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 9)
		{

		M_DrawPic ( (692-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 8)
		{

		M_DrawPic ( (698-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 7)
		{

		M_DrawPic ( (704-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 6)
		{

		M_DrawPic ( (710-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 5)
		{

		M_DrawPic ( (716-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 4)
		{

		M_DrawPic ( (722-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 3)
		{

		M_DrawPic ( (728-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 2)
		{

		M_DrawPic ( (734-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 1)
		{

		M_DrawPic ( (740-smgb->width)/2, 50, smgb);
		}
		if (cl.stats[STAT_AMMO] >= 0)
		{
...
*/
	if (cl.stats[STAT_ACTIVEWEAPON] == IT_GRENADE_LAUNCHER)
	{
		...
		M_DrawPic ( (665-sniperb->width)/2, 40, sniperb);
		}

		if (cl.stats[STAT_AMMO] >= 3)
		{

		M_DrawPic ( (735-sniperb->width)/2, 40, sniperb);
	}
	if (cl.stats[STAT_AMMO] >= 2)
	{

	M_DrawPic ( (665-sniperb->width)/2, 47, sniperb);
	}

	if (cl.stats[STAT_AMMO] >= 1)
	{

	M_DrawPic ( (735-sniperb->width)/2, 47, sniperb);
	}
...
*/
	...
	M_DrawPic ( (620-clock[round]->width)/2, 30, clock[round] );
...
*/
	...
	{
	sprintf (up,"Wave %d", round);
	Sbar_DrawString (385, -5, up);
	}
/*
=============================================
Firefight Lives
...
*/
	...
	Sbar_DrawString (380, 10, up);
	}
}
void Sbar_DrawGametype (void)
{
if (deathmatch.value == 1)
Sbar_DrawString (385, -17, "Slayer");
if (deathmatch.value == 2)
Sbar_DrawString (385, -17, "Firefight");
if (deathmatch.value == 3)
Sbar_DrawString (385, -17, "Swat");
}
...
//	else if (sb_lines)
   ...
   // armor
	...
	if (cl.stats[STAT_ARMOR] <= 0)
        ...
        {
            if (kurok)
                Sbar_DrawNum2 (620/2, 10, cl.stats[STAT_ARMOR], 3, cl.stats[STAT_ARMOR] <= 50);
            else
