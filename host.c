...
cvar_t	fraglimit = {"fraglimit","50",false,true};
cvar_t	timelimit = {"timelimit","60",false,true};
...
void	Host_FindMaxClients (void)
{
	...
	if (svs.maxclients > 1)
	{
		...
		else if (deathmatch.value == 2.0)		//Firefight
			{
			Cvar_SetValue ("deathmatch", 2.0);
			Cvar_Set ("teamplay", "1");
			}
		else if (deathmatch.value == 3.0)		//Swat
			{
			Cvar_SetValue ("deathmatch", 3.0);
			}
		else
			Cvar_SetValue ("deathmatch", 1.0);	//Slayer
	}
	else
