...
=============
*/
void Menu_DrawAlphaPic (int x, int y, qpic_t *pic, float alpha)
{
  if(kurok)
...
//        M_DrawTransPic (72, 32, Draw_CachePic ("gfx/menu/multi_0.lmp") );
    	...
    	M_DrawPic ((0-a->width)/2, 152, a);

	    M_DrawCheckbox ((320/2) - ((3*8)/2), 168, tcpipAvailable && tcpipAdhoc);

    	if (m_multiplayer_cursor == 5)
	        M_PrintWhite ((0/2) - ((8*8)/2), 184,  "Add Bot");
        else
	        M_Print ((0/2) - ((8*8)/2), 184,  "Add Bot");

    	if (m_multiplayer_cursor == 6)
	        M_PrintWhite ((0/2) - ((12*8)/2), 192,  "Add Team Bot");
        else
	        M_Print ((0/2) - ((12*8)/2), 192,  "Add Team Bot");

	    if (m_multiplayer_cursor == 7)
	        M_PrintWhite ((320/2) - ((10*8)/2), 200,  "Remove Bot");
        else
	        M_Print ((0/2) - ((10*8)/2), 200,  "Remove Bot");

	    if (m_multiplayer_cursor == 8)
	        M_PrintWhite ((320/2) - ((26*8)/2), 216,  "Co-op Player Camera Change");
        else
	        M_Print ((0/2) - ((26*8)/2), 216,  "Co-op Player Camera Change");

	    if (serialAvailable || ipxAvailable || tcpipAvailable)
	    	return;
	    M_PrintWhite ((0/2) - ((27*8)/2), 232, "No Communications Available");
    }
	else
	{
	    M_DrawTransPic (16, 4, Draw_CachePic ("gfx/qplaque.lmp") );
...
level_t		kuroklevels[] =
{
	...
	{"Maze", "Small Test place"},	//1
	{"Trap-Bridges", "Jukki"},
	{"Station-118", "A small 2v2 base"},
	{"Cliff", "Flaming Ice best map"},
	{"Chill", "Jomart-2000 Halo 1 remake"},
	{"Lockout", "Not complete"},
    	{"prisoner", "Jomart2000"},
	{"Rat", "Jomart2000"},


	{"Needle", "How many points can you rake up?"}, 	// 9

	{"buildings", "Firefight Buildings"},		//10
	{"spawn", "Elite Firefight"},
	{"Desert", "Desert Firefight"},

	{"Spiders-Web", "Solitude"},	//13
	{"Manic", "Solitude"},
	{"Narrowed", "Solitude"}
};
...
//	int		r;
	...
	M_PrintWhite (-70, 170, "-Current Settings-");
...
if (deathmatch.value == 0)
	...
	if (noexit.value == 1)
		...
		if (deathmatch.value == 2)
			M_PrintWhite (160, 170, "Firefight Settings");
    if(kurok)
    {
        M_DrawTextBox (152, 120, 18, 1);
        M_PrintWhite (160, 128, "Slayer Options");
    }
...
void M_NetStart_Change (int dir)
{
	...
	case 2:
		Cvar_SetValue ("deathmatch", deathmatch.value + dir);
	...
	case 6:
		...
		if (timelimit.value > 600)
			Cvar_SetValue ("timelimit", 0);
		if (timelimit.value < 0)
			Cvar_SetValue ("timelimit", 600);
		break;
