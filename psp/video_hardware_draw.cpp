...
void Draw_Crosshair(void)
{
	...
	if (kurok)
	{
	    ...
	    {
            if (cl.stats[STAT_ACTIVEWEAPON] == IT_AXE)                  // Energy Sword
				pic = Draw_CachePic ("gfx/ch/ch_sword.lmp");

	        else if (cl.stats[STAT_ACTIVEWEAPON] == IT_SHOTGUN)         // Pistol
				if (cl_ch_red.value == 1)
				...
				if (cl_ch_red.value == 1)
				pic = Draw_CachePic ("gfx/ch/ch_rshottie.lmp");
				else
				pic = Draw_CachePic ("gfx/ch/ch_shottie.lmp");

	        else if (cl.stats[STAT_ACTIVEWEAPON] == IT_NAILGUN)         // Assualt rifle
			if (cl_ch_red.value == 1)
				pic = Draw_CachePic ("gfx/ch/ch_rar.lmp");
			else
				pic = Draw_CachePic ("gfx/ch/ch_ar.lmp");
				...
				if (cl_ch_red.value == 1)
				pic = Draw_CachePic ("gfx/ch/ch_rsniper.lmp");
				else
				pic = Draw_CachePic ("gfx/ch/ch_sniper.lmp");

			else if (cl.stats[STAT_ACTIVEWEAPON] == IT_ROCKET_LAUNCHER) // Rocket launcher
				if (cl_ch_red.value == 1)
				pic = Draw_CachePic ("gfx/ch/ch_rrl.lmp");
				else
				pic = Draw_CachePic ("gfx/ch/ch_rl.lmp");

			else if (cl.stats[STAT_ACTIVEWEAPON] == IT_LIGHTNING)     
				if (cl_ch_red.value == 1)
				pic = Draw_CachePic ("gfx/ch/ch_rppist.lmp");
				else
				pic = Draw_CachePic ("gfx/ch/ch_ppist.lmp");
